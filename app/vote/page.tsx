...
import { useVerification } from "../page";
...
const page = () => {
  ...
  const handleCandidateSelect = (categoryId: string, candidateName: string) => {
  ...
  return (
    <div>
      <div className="flex flex-col items-center justify-center font-sans pt-7 gap">
        ...
        <ul>
          {categories.map((category) => (
            <li
              key={category.id}
              className="py-2 my-3 rounded-lg lg:my-4 lg:px-7"
            >
                  ...
                  {Object.values(category.candidates).map(
                    (candidate, index) => (
                      <div
                        key={candidate.name}
                        onClick={() => {
                          handleCandidateSelect(category.name, candidate.name);
                        }}
                        className={` ${
                          selectedCandidates[category.name] === candidate.name
        ...
        <button
          className="bg-(--primary) text-sm md:text-xl text-(--secondary) h-10 lg:h-10 w-full rounded-xl  font-bold "
